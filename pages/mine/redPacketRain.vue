<template>
  <div class="mescroll">
    <canvas id="rain" @click="judgePack($event)" />
    <img
      id="hotbag"
      ...
      hidden="true"
    >
    <button class="button--green" @click="init">
      开始
    </button>
  </div>
</template>
...
export default {
  ...
  mounted () {
    const can = document.querySelector('#rain')
  ...
  methods: {
    ...
    // 创建雨滴
    createRain (i) {
      const rain = new Rain(this.W, this.H, this.hotbagW, this.hotbagH)
      rain.init(i)
      rain.draw()
    ...
    judgePack (event) {
      ...
      this.rainArray.forEach((item, index) => {
        if (
          item.alive &&
          item.x <= mouseX &&
          mouseX - item.x < this.hotbagW * 2 &&
          item.y - this.hotbagH <= mouseY &&
          mouseY - item.y < this.hotbagH * 2
        ) {
          item.destory()
        }
      })
...
function Rain (windowW, windowH, hotbagW, hotbagH) {
  ...
  this.draw = function () {
    ...
    if (this.y < this.maxH && this.alive) {
      ...
      canContent.drawImage(this.src, this.x, this.y, 2 * hotbagW, 2 * hotbagH)
      canContent.stroke()
